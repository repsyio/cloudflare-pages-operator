...
```sh
docker build -t <registry>/cfpo:0.1.0 .
docker build -t <registry>/cfpo-deployer:0.1.0 deployer/
docker push <registry>/cfpo:0.1.0
docker push <registry>/cfpo-deployer:0.1.0
```
...
helm install cfpo charts/cfpo -n cfpo-system --create-namespace \
  ...
  --set deployer.image=<registry>/cfpo-deployer:0.1.0 \
